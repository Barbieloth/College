            ...
            try {
                switch (choice) {
                    ...
                    case "2": {
                        ...
                        vec.add(idx, sc.nextLine());
                        System.out.println("Елемент вставлено.");
                    } break;
                    case "3": {
                        System.out.print("Введіть індекс для видалення: ");
                        int idx = Integer.parseInt(sc.nextLine());
                        String removed = vec.remove(idx);
                        System.out.println("Видалено елемент: " + removed);
                    } break;
                    case "4": {
                        System.out.print("Введіть індекс: ");
                        int idx = Integer.parseInt(sc.nextLine());
                        System.out.println("Результат: " + vec.get(idx));
                    } break;
                    case  "5": {
                        System.out.println("Поточний розмір: " + vec.size());
                    } break;
                    case "6": {
                        System.out.println("Поточна ємність буфера: " + vec.capacity());
                    } break;
                    case "7": {
                        for (int i = 0; i < 10; i++) {
                            vec.add("яблучкі" + (i + 1));
                        }
                        System.out.println("Новий розмір: " + vec.size());
                        System.out.println("Нова ємність: " + vec.capacity());
                    } break;
                    case "0": {
                        running = false;
